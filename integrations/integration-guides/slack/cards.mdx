...
2. In the **Timestamp** field, add the message's `ts` property. This is a unique identifer for individual Slack messages — you can retrieve a message's `ts` programatically using the [Slack API](https://api.slack.com/messaging/retrieving#conversations). 
3. In the **Channel** field, add the channel the message was sent in.
...
3. Store the result in a [variable](/learn/reference/variables/overview). The result wil contain the user's:
