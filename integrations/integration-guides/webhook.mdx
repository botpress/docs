...
icon: '/integrations/integration-guides/assets/icons/webhook.svg'
---
...
<Tip>
  You can [test the webhook in the Studio's emulator](/studio/concepts/triggers#test-a-trigger) to make sure you're handling requests properly.
</Tip>
...
3. Now, you can [read the event's payload](/studio/guides/advanced/event-properties) to access information about the webhook request:
...
The contents of `event.payload` are overwritten as soon as the bot or the user sends a new message. If you need to access a webhook request's data later, consider storing it in a [variable](/studio/concepts/variables/overview).
