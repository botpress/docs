...
```bash
# staging config
REDIS_URL=redis://host:port # the same value for both configs
BP_REDIS_SCOPE=staging # a unique value for this config 
```

```bash
# production config
REDIS_URL=redis://host:port # the same value for both configs
BP_REDIS_SCOPE=production # a unique value for this config 
```
