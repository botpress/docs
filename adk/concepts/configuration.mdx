...
<Note>
  `config:get` reads persisted values only and does not resolve schema defaults.
</Note>
...
<Note>
  There is no dedicated secrets API in the ADK. For integration-specific credentials (like OAuth tokens), configure them in the Control Panel during `adk dev` or via the Botpress dashboard.
</Note>
...
Integration settings (API keys, tokens, etc.) are configured in the Control Panel during `adk dev`, not in `agent.config.ts`. See [Managing Integrations](/adk/managing-integrations) for details.
