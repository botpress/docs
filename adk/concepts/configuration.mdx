...
Run `adk config` to interactively set all configuration values:
...
```bash
adk config:set apiKey sk-my-api-key
adk config:get apiKey
```
...
```bash
adk config --prod
adk config:set apiKey sk-prod-key --prod
adk config:get apiKey --prod
```
