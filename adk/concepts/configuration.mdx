...
You can use `process.env` directly in `agent.config.ts` and throughout your agent code:
...
The ADK also injects its own environment variables during `adk dev` and `adk run` (such as `ADK_CONFIGURATION` and `ADK_BOT_ID`), but these are managed internally.

<Note>
  There is no dedicated secrets API in the ADK. For integration secrets (like API keys for Slack or BambooHR), set them via the Botpress dashboard. They're stored server-side and preserved during deployments.
</Note>
