...
Import `bot` from `@botpress/runtime` to access bot-level state from anywhere in your agent:
...
Import `user` from `@botpress/runtime` to access per-user state:
...
export default new Conversation({
  ...
  handler: async ({ state }) => {
    // state.activeWorkflow is a full WorkflowInstance when loaded
    if (state.activeWorkflow?.status === "completed") {
