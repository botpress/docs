...
<Steps>
  ...
  <Step>
    Go to **Settings → Deflecting Bots**.
  </Step>
  <Step>
    Add your bot.
  </Step>
...
Wrap `startHitl` in an `Autonomous.Tool` so the model can decide when to escalate. Create a file under `src/tools/`:
...
Pass the tool to `execute()` and instruct the model when to use it:
...
If the user needs help beyond your capabilities or explicitly asks for a support agent, use the escalateToHuman tool. After calling it, do not send any message.`,
