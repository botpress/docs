...
Integration configuration is stored on Botpress Cloud, not in your project files. Your dev bot and prod bot each hold their own, so you can point an integration at different API keys per environment. This is separate from agent secrets, which your code reads directly.
