...
The ADK ships skills that teach these assistants how to work with ADK primitives, so they can scaffold files, add integrations, run evals, and debug issues with the right conventions, without any extra setup from you.
  ...
  <Step title="Build with your assistant">
    ...
    <AccordionGroup>
      <Accordion title="Add a feature">
        ```txt
        Turn this hello-world agent into a recipe assistant that suggests dishes based on ingredients the user has.
        ```

        Loads `botpress-adk` to rewrite the conversation handler's instructions and add any helper tools the agent needs.
      </Accordion>
