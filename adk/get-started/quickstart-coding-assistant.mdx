  ...
  <Step title="Build with your assistant">
    ...
    <AccordionGroup>
      <Accordion title="Add a feature">
        ```txt
        Add a coin-flip tool the agent can call when users ask for a random yes/no answer.
        ```

        Loads `botpress-adk` to scaffold the tool and wire it into the conversation handler.
      </Accordion>
