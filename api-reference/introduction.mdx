...
Except for the Chat API, all APIs mentioned below are located at the following URL: `https://api.botpress.cloud/v1`.

## Chat API

The Chat API enables interaction with your deployed bots via HTTP requests. Unlike other APIs, which are deployed as static services for all users, the Chat API is unique because it is an integration rather than a service. To use the Chat API in your bot, you first need to install the Chat Integration. As stated above, the Chat API is the only API documented here that is not available under the `https://api.botpress.cloud/v1` URL. It can instead be found at `https://chat.botpress.cloud/`. For more details, refer to the [Chat API documentation](./chat-api).
