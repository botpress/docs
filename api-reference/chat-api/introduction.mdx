...
<Note>
Since the Chat API is an integration, all users, conversations and messages created using the Chat API belong to the Chat Integration. This mean, you can't use the Chat API to fetch conversations that occured in WhatsApp or in the Webchat. For that, take a look at the [Runtime API](../runtime-api/getting-started.mdx).
</Note>
