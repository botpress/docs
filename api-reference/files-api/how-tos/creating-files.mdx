...
By default the file URL returned will be temporary and change on each request to this endpoint, and will expire after a short period of time and thus shouldn't be stored long-term, but if the file was created with a `public_content` [access policy](/api-reference/files-api/openapi/upsertFile#body-access-policies) then this URL will be permanent and can be stored long-term.
...
To add a file to a [Knowledge Base](/studio/concepts/knowledge-base/introduction), you need to set custom tags when making your request.

Here's an example. Make sure the `index` parameter is set to `true`. Otherwise, you won't be able to open the file in Studio:

<CodeGroup>

  ```ts highlight={10-14} Using the Botpress client
  import { Client } from '@botpress/client';
  ...
  ```ts highlight={17-21} Using an HTTP request
