...
Bots can't be accessed directly from the Runtime API. Instead, they are manageable in [the Admin API](../admin-api/concepts.mdx#bot). However, they are still a core concept of the Runtime API. Bots are software applications that handle events and messages and take actions on them.

## Integration

Integrations can't be accessed directly from the Runtime API. Instead, they are manageable in [the Admin API](../admin-api/concepts.mdx#integration). However, they are still a core concept of the Runtime API. Integrations are software components that integrate Botpress with external services. They can be used to send and receive messages, events, and other data between Botpress and the external service. Integrations are defined in the Botpress Admin API and can be managed through the Admin API.
