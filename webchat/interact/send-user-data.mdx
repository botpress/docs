...
Use the `window.botpress.updateUser` method to update user data after your website has initialized Webchat:
...
To provide user data as soon as Webchat is ready, listen for Webchat's initialization event and call `updateUser` as soon as it fires:
...
Once you've provided Webchat with user data, you can use the [Get User Data](/learn/reference/cards/webchat#get-user-data) Card to access it in the [Studio](/learn/reference/introduction):
...
Use the `getUser` method to access the current user data programmatically:
