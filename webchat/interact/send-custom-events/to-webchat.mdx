...
<Steps>
    <Step title="Send an event to Webchat">
        ...
        First, set up an event to send Webchat from your website's source code. Use the [`window.botpress.sendEvent`](/webchat/interact/reference#sendevent) method:
    ...
    <Step title="Add a Custom Trigger">
      ...
      Next, add a [Custom Trigger](/studio/concepts/triggers#custom-trigger) to your bot's Workflow:
    ...
    <Step title="Test your trigger">
        ...
        2. Select the <Icon icon="play" /> button to test the Custom Trigger's behaviour in the Studio's [Emulator](/studio/concepts/emulator).
    </Step>
</Steps>
