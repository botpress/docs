...
The [Summary Agent](/learn/reference/agents/summary-agent) listens to all new messages in the conversation to build a summary and a transcript.
...
The `summary` variable is useful if you don't need to have many details about the conversation, especially because some of them get lengthy and if you may not want all that data. The `transcript` variable is the best way to have the full history of the conversation in a single text without needing [Hooks](/learn/reference/hooks) or other workarounds. Using the Summary Agent for exporting conversations is also great because you have access to the history during the very conversation so you don't need to make manual or scheduled requests to our API from external services.
...
current session, so if the user is resuming a conversation, you won't have the previous messages in the history. To get the whole conversation at any moment, use the [API method for exporting conversations](/learn/guides/advanced/appendix/exporting-data/exporting-raw-conversations-with-the-botpress-api)

# Using Hooks

[Hooks](/learn/reference/hooks) are functions that are executed under the hood every time there's a new message from the user or from the bot. You can create hooks that build the conversation history as it happens. They work similarly to the Summary Agent but with hooks you can customize how the history is built and there's no limitation for its size (with the exception of the 128KB max session size).
...
won't have the previous messages in the history. To get the whole conversation at any moment, use the [API method for exporting conversations](/learn/guides/advanced/appendix/exporting-data/exporting-raw-conversations-with-the-botpress-api)
...
Now that you have the conversation history, you can add an [Execute Code card](/learn/reference/cards/execute) to manipulate the variable as you prefer, for example sending it to an API or via email:
...
You could add this card to the [Conversation End](/learn/reference/workflows#conversation-end) Workflow so that the full session is sent when the conversation ends. (The bot will only enter the End flow if there's a explicit Transition to an End node)
</Info>
