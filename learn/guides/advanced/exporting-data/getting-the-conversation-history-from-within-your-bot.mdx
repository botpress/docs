...
current session, so if the user is resuming a conversation, you won't have the previous messages in the history. To get the whole conversation at any moment, use the [API method for exporting conversations](/learn/guides/advanced/exporting-data/exporting-raw-conversations-with-the-botpress-api)
...
won't have the previous messages in the history. To get the whole conversation at any moment, use the [API method for exporting conversations](/learn/guides/advanced/exporting-data/exporting-raw-conversations-with-the-botpress-api)
...
Now that you have the conversation history, you can add an [Execute Code card](/learn/reference/cards/execute-code) to manipulate the variable as you prefer, for example sending it to an API or via email:
...
You could add this card to the [Conversation End](/learn/reference/workflows#conversation-end) Workflow so that the full session is sent when the conversation ends. (The bot will only enter the End Workflow if there's an explicit Transition to an End node)
</Info>
