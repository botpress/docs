...
Any Hook of this type will execute its code before your bot processes an incoming `event`.
...
This Hook executes after the conversation has **explicitly ended** (transitioned to an [End Node](/learn/reference/nodes/introduction#end-node))
...
This Hook executes after a full turn has completed — meaning the bot has received user input, processed it, and responded.
