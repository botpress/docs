...
Like [bot variables](/learn/reference/variables/scopes/bot), configuration variables are accessible across all Workflows and conversations. The key difference is that configuration variables are **encrypted**. This means an Autonomous Node can't read or display information you store in a configuration variable.
...
You can use `env.variablename` to read a configuration variable. For example, in an [Execute Code](/learn/reference/cards/execute-code) Card:
