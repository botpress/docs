...
The `Simulate timeout event` button triggers the [Timeout Workflow](/learn/reference/workflows#timeout) so you can check how the bot behaves when the conversation expires. It's available at the top right corner of the emulator behind the 3 dots.

# In-line Logs

The Emulator shows the status of the conversation and the different steps that are being or have been executed under the hood to get the bot's response. They're a simpler version of the [Event Debugger](/learn/reference/debugger-logs-json)'s summary. Bot events, and card-processing results are shown this way, only in the emulator.

## Bot-events

* Conversation started: The user has started the conversation by sending a message or a [Trigger](/learn/get-started/dashboard/bot/events) Event has been triggered.
* Conversation ended: The conversation flowed to an End node and has gone through the [Conversation End Workflow](/learn/reference/workflows#conversation-end), or it has ended by not having possible transitions.
* Error occurred: An irrecoverable error has occurred and the conversation has flowed to the [Error](/learn/reference/workflows#error) . The error message is visible by hovering the status.
...
* Answer found in Knowledge Base: The bot has gone through the [Knowledge Bases](/learn/reference/knowledge-base/introduction) and found an answer to the user's question. Clicking it will take you to the [Logs tab](/learn/reference/debugger-logs-json) where you can see in details what was sent to the AI and the raw output.
...
* Knowledge Base answer: Text displayed automatically after a question without the need of a Text card. It will contain the citations to help you [debug the answers](/learn/reference/knowledge-base/introduction#debugging).

### Execute

* Executing code: The bot is executing some custom code from an [Execute Code card](/learn/reference/cards/execute-code).
...
* Capturing Variable: The bot is currently trying to extract information from the user message with a [Capture card](/learn/reference/cards/capture-information) to optionally save it in a [variable](/learn/reference/variables).
* Captured variable: The bot has successfully extracted information from the user message and saved it in a variable.
* Capture variable failed: The bot wasn't able to extract information and has reached the limit of [retries](/learn/reference/cards/capture-information#retries).
