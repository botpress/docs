...
The Emulator shows the status of the conversation and the different steps that are being or have been executed under the hood to get the bot's response. They are a simpler version of the [Event Debugger](/learn/reference/debugger-logs-json)'s summary. Bot events, and card-processing results are shown this way, only in the emulator.
...
* Error ocurred: An irrecoverable error has ocurred and the conversation has flowed to the [Error](/learn/reference/workflows#error) . The error message is visible by hovering the status.
