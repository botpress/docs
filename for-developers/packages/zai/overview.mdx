...
Zai is built on [Zod schemas](https://zod.dev/) and the Botpress API. We recommend using it with the [Botpress SDK](/for-developers/sdk/overview) to help process LLM inputs and outputs.
...
This also installs the [official Botpress client](https://www.npmjs.com/package/@botpress/client) and the [internal Zui library](https://www.npmjs.com/package/@bpinternal/zui), which are necessary to use Zai.
...
Then, create a new `Zai` object and pass in the `Client`:

```ts
const zai = new Zai({ client })
```

Now, you can use Zai's methods to process LLM outputs.
...
```ts
const comments = ['Great product, I love it!', 'This is terrible spam content', 'Very helpful review, thank you']
...
const rewritten = await zai.rewrite(original, 'Make this sound more professional and formal')
...
```ts
const blogPost = await zai.text('Write a brief introduction about the benefits of AI in customer service', {
  length: 200,
})
