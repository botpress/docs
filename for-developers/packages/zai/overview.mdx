...
Use the [`sort`](/for-developers/packages/zai/reference#sort) method to order items using natural language instructions:
...
Use the [`rate`](/for-developers/packages/zai/reference#rate) method to evaluate items on a 1-5 scale:
...
Use the [`group`](/for-developers/packages/zai/reference#group) method to categorize items into groups:
...
Use the [`answer`](/for-developers/packages/zai/reference#answer) method to answer questions from documents with source citations:
...
Use the [`patch`](/for-developers/packages/zai/reference#patch) method to make surgical code edits across one or many files. Instead of  regenerating entire files, it makes precise, minimal changes while preserving formatting and context:
