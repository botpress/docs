...
Use the `step` function to create checkpoints in long-running workflows. Steps are persisted, so if a workflow is interrupted, it can resume from the last completed step.
...
The `step` object provides many additional methods for workflow control including `sleep`, `map`, `batch`, and more.

<Card title="Workflow steps" icon="list-check" href="/for-developers/adk/concepts/workflows/steps">
