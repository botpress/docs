...
`https://slack.com/oauth/v2/authorize?client_id=$CLIENT_ID&state=$WEBHOOK_ID&redirect_uri=https://webhook.botpress.cloud/oauth&scope=channels:history`

The state query param must contain the webhook ID of your integration. This is used to identify the integration when the OAuth flow is completed. The redirect URI must be set to `https://webhook.botpress.cloud/oauth`.
...
Note: you might have to add global secrets to your integrations to provide credentials to the OAuth application. For that your can refer to the [Secrets](./concepts#secrets) guide.
