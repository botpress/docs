...
sidebarTitle: How to install packages
---

A package is an SDK project, such as a plugin, integration, or interface. Packages are installed into an SDK project using the `add` command of Botpress's CLI. For a bot, you can add a plugin, an integration, or an interface. For an integration, you can add an interface.
...
Installing interface packages is useful for integration development. It allows an integration developer to implement an interface.
