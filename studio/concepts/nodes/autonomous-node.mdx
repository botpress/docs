...
By default, your Autonomous Node runs in a loop on every user message until one of its [exit conditions](#configuration) are met. If you disable this option, the Autonomous Node will only process one user message before transitioning to the next Node.
