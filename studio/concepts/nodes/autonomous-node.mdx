...
- **Vision Agent**: This section configures whether your Autonomous Node uses the [Vision Agent](/studio/concepts/agents/vision-agent) to extract text content from incoming images. Toggle **Extract from Incoming Images** to enable or disable this option.
...
<Tabs>
  <Tab title="Default tools">
    ...
    This prompt narrows when each search tool runs so the Node does not pull unrelated content:
  ...
  <Tab title="Transition tools">
    ...
    This prompt combines `global.Message` with `workflow.transition` when the user picks a numeric option. Connect each option with another Node in your Workflow first so `workflow.transition` is valid:
