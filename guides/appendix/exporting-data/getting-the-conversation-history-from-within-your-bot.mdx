...
The [Summary Agent](https://botpress.com/docs/agents#summary-agent) listens to all new messages in the conversation to build a summary and a transcript.
...
The `summary` variable is useful if you don't need to have many details about the conversation, especially because some of them get lengthy and if you may not want all that data. The `transcript` variable is the best way to have the full history of the conversation in a single text without needing [Hooks](https://botpress.com/docs/hooks) or other workarounds. Using the Summary Agent for exporting conversations is also great because you have access to the history during the very conversation so you don't need to make manual or scheduled requests to our API from external services.
...
won't have the previous messages in the history. To get the whole conversation at any moment, use the [API method for exporting conversations](https://botpress.com/docs/docs/exporting-raw-conversations-with-the-botpress-api)
...
Now that you have the conversation history, you can add an [Execute Code card](https://botpress.com/docs/docs/execute-code) to manipulate the variable as you prefer, for example sending it to an API or [via email](https://botpress.com/docs/docs/tips-tricks#send-emails-using-sendgrid-api):
