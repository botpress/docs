...
It's also recommended to save these occurrences somehow so you can troubleshoot them later. You could use [Tables](/guides/studio/interface/tables) to create a database of these occurrences, containing the conversation history provided by the [Summary Agent](/guides/studio/interface/agents/summary-agent) and other relevant information.
</Info>

## Timeout

The **Timeout Flow** is triggered when the chatbot doesn't receive a response from the user for a certain period of time. Check out [this article about the Timeout settings](/guides/studio/interface/chatbot-settings#inactivity-timeout) to learn more.

This can happen if the user gets distracted with something else or if there is a delay in their internet connection.

The Timeout Flow is important because it helps prevent the chatbot from appearing unresponsive or disconnected, which can lead to user frustration and disengagement. Learn more about the [Inactivity Timeout setting](/guides/studio/interface/chatbot-settings#inactivity-timeout).
...
Learn more about using [workflow variables](/guides/studio/interface/variables#workflow-variables)
